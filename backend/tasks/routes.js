...
router.get('/list/sprints/:sprint_id', verifyToken, (request, response) => {
  ...
  database.all(`SELECT * FROM Tasks WHERE sprint_id = ?`, [sprint_id], (err, tasks) => {
...
router.get('/:task_id', verifyToken, (request, response) => {
  ...
  database.get(`SELECT * FROM Tasks WHERE task_id = ?`, [task_id], (err, task) => {
    if (err) return response.status(500).json({ error: err.message });
...
router.post('/sprints/:sprint_id/create', verifyToken, (request, response) => {
  ...
  const query = `
    ...
    VALUES (?, ?, ?, ?)
  `;
  database.run(query, [task_name, sprint_id, created_by, status || 'TODO'], function (err) {
...
router.post('/:task_id/assign', verifyToken, (request, response) => {
  ...
  if ((assigned_to_user && assigned_to_team) || (!assigned_to_user && !assigned_to_team)) {
...
router.delete('/:task_id/delete', verifyToken, (request, response) => {
  const { task_id } = request.params;
  database.run(`DELETE FROM Tasks WHERE task_id = ?`, [task_id], function (err) {
    if (err) return response.status(500).json({ error: err.message });
    if (this.changes === 0) return response.status(404).json({ error: 'Task not found' });
...
router.get('/:task_id/subtasks', verifyToken, (request, response) => {
  const { task_id } = request.params;
  database.all(`SELECT * FROM Subtasks WHERE task_id = ?`, [task_id], (err, subtasks) => {
    if (err) return response.status(500).json({ error: err.message });
...
router.post('/:task_id/subtasks/create', verifyToken, (request, response) => {
  const { task_id } = request.params;
  const { subtask_name, created_by, status } = request.body;
  database.run(
    `INSERT INTO Subtasks (subtask_name, task_id, created_by, status)
     VALUES (?, ?, ?, ?)`,
    [subtask_name, task_id, created_by, status || 'TODO'],
    function (err) {
      if (err) return response.status(400).json({ error: err.message });
...
router.put('/subtasks/:subtask_id/update', verifyToken, (request, response) => {
  ...
  database.run(
    ...
    function (err) {
      if (err) return response.status(500).json({ error: err.message });
      if (this.changes === 0) return response.status(404).json({ error: 'Subtask not found' });
...
// 4. Delete sub-task
router.delete('/subtasks/:subtask_id/delete', verifyToken, (request, response) => {
  const { subtask_id } = request.params;
  database.run(`DELETE FROM Subtasks WHERE subtask_id = ?`, [subtask_id], function (err) {
    if (err) return response.status(500).json({ error: err.message });
    if (this.changes === 0) return response.status(404).json({ error: 'Subtask not found' });
