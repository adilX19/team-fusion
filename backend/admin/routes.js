...
// CREATE A NEW USER
router.post("/create-user", verifyToken, (request, response) => {
  const { username, email, owner_id, firstname, lastname, password, role } =
    request.body;
  ...
  database.get(
    ...
    async (err, user) => {
      ...
      database.run(
        "INSERT INTO Users (username, email, owner_id, firstname, lastname, password, role) VALUES (?, ?, ?, ?, ?, ?, ?)",
        [username, email, owner_id, firstname, lastname, hashedPassword, role],
        function (err) {
...
// GET ALL THE AVAILABLE USERS BY OWNER
router.get("/users/owned", verifyToken, (request, response) => {
...
router.put("/users/:id", verifyToken, (request, response) => {
  const userId = request.params.id;
  const { username, email, is_superuser, is_staff, reports_to, role } =
    request.body;
...
router.delete("/users/:id", verifyToken, (request, response) => {
  ...
  database.get(
    ...
    (err, user) => {
      ...
      database.run("DELETE FROM Users WHERE user_id = ?", [userId], (err) => {
        if (err)
          return response
            .status(500)
            .json({ message: "Database error", error: err.message });
